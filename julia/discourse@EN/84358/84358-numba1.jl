...
# Each cycle requires a different size of data to be cached
...
# inplace op
function fun!(tmp::FunTemp, _data, c, dataSize::Int64)
    x = @view _data[:,1]
    y = @view _data[:,2]
    z = @view _data[:,3]

    for i in 1:dataSize
        @inbounds for k in 1:dataSize
            xx = x[k] - x[i]
            yy = y[k] - y[i]
            zz = z[k] - z[i]
...
function main(st,data::Array{Float64,2},p::Array{Int64,1},m)
    ...
    dd = zeros(20,20);
    @views @inbounds for i in p
        id = st[i]
        t_data = data[id,:];
        t_data_size, _ = size(t_data)
        
        fun!(tmp1, t_data, m, t_data_size)
        A,A1,B1,C1,A2,B2,C2 = viewFunTemp(tmp1, t_data_size)
