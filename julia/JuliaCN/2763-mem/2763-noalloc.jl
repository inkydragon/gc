...
bound_tmp_598 = Array{Float64}(undef, 598)
bound_tmp_600 = Array{Float64}(undef, 600)
bound_tmp_1200 = Array{Float64}(undef, 1200)
...
for iterStep = 1:iterStepMax
    ...
    streaming(Q, cx, cy, f)
    boundaryCondition(nx, ny, u, v, ρ, U, f,  bound_tmp_598, bound_tmp_600, bound_tmp_1200)
...
# @views @. f[1, 2:ny-1, 5] = f[1, 2:ny-1, 7] - 0.5 * (f[1, 2:ny-1, 2] - f[1, 2:ny-1, 4]) + ρ[1, 2:ny-1] * U / 6.0
# @views @. f[1, 2:ny-1, 8] = f[1, 2:ny-1, 6] + 0.5 * (f[1, 2:ny-1, 2] - f[1, 2:ny-1, 4]) + ρ[1, 2:ny-1] * U / 6.0
@views @. bound_tmp_598 = f[1, 2:ny-1, 3] + ρ[1, 2:ny-1] * U * 2.0 / 3.0
@views @. f[1, 2:ny-1, 1] = bound_tmp_598
@views @. bound_tmp_598 = f[1, 2:ny-1, 7] - 0.5 * (f[1, 2:ny-1, 2] - f[1, 2:ny-1, 4]) + ρ[1, 2:ny-1] * U / 6.0
@views @. f[1, 2:ny-1, 5] = bound_tmp_598
@views @. bound_tmp_598 = f[1, 2:ny-1, 6] + 0.5 * (f[1, 2:ny-1, 2] - f[1, 2:ny-1, 4]) + ρ[1, 2:ny-1] * U / 6.0
@views @. f[1, 2:ny-1, 8] = bound_tmp_598
...
# @views @. f[nx, :, 6] = f[nx-1, :, 6]
# @views @. f[nx, :, 7] = f[nx-1, :, 7]
@views @. bound_tmp_600 = f[nx-1, :, 3]
@views @. f[nx, :, 3] = bound_tmp_600
@views @. bound_tmp_600 = f[nx-1, :, 6]
@views @. f[nx, :, 6] = bound_tmp_600
@views @. bound_tmp_600 = f[nx-1, :, 7]
@views @. f[nx, :, 7] = bound_tmp_600
...
# @views @. f[:, 1, 5] = f[:, 1, 7]
# @views @. f[:, 1, 6] = f[:, 1, 8]
@views @. bound_tmp_1200 = f[:, 1, 4]
@views @. f[:, 1, 2] = bound_tmp_1200
@views @. bound_tmp_1200 = f[:, 1, 7]
@views @. f[:, 1, 5] = bound_tmp_1200
@views @. bound_tmp_1200 = f[:, 1, 8]
@views @. f[:, 1, 6] = bound_tmp_1200
...
copy!(bound_tmp_1200, @view f[:, ny, 2])
copy!(f[:, ny, 4], bound_tmp_1200)
copy!(bound_tmp_1200, view(f, :, ny, 6))
copy!(f[:, ny, 8], bound_tmp_1200)
copy!(bound_tmp_1200, @view f[:, ny, 5])
copy!(f[:, ny, 7], bound_tmp_1200)
