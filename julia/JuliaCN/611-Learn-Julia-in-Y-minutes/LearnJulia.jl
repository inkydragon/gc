...
# 字符串使用 UTF-8 编码
# 可以像取数组取值一样用 index 取出字符串中的对应字符
# 但只有在字符串仅由 ASCII 字符构成时，字符串才能够被安全的引索
ascii("This is a string")[1]  
# => 'T': ASCII/Unicode U+0054 (category Lu: Letter, uppercase)
# Julia 的引索值从 1 开始计数
# 另外，推荐使用迭代来处理字符串 (map, for loops 等)


# $ 用于插入字符串
"2 + 2 = $(2 + 2)" # => "2 + 2 = 4"
# 你可以在括号中放入任何 Julia 表达式
...
# 变量名必须以下划线或字母开头
# 之后使用字母、数字、下划线以及感叹号都是合法的
SomeOtherVar123! = 6  # => 6

# 你也可以使用特定的 Unicode 字符
☃ = 8 # => 8
# 这样使用数学记号就十分方便
...
#
# * 类型名应以大写字母开头，单词间用驼峰命名法分隔，不使用下划线。
#
# * 函数和宏的名称小写，不用下划线。
#
# * 会改变输入的函数名字末尾加感叹号「!」。
#   这些函数有时称为 mutating functions 或 in-place functions
#

# 数组储存着一系列值，用 1 到 n 的整数作为引索
...
# end 是数组最后一个引索的简称，它可以用在任何索引表达式中
a[end]  # => 6
...
# 短 λ 表达式(stabby lambda syntax) 可以用来创建匿名函数
(x -> x > 2)(3)  # => true
...
# DataType 是代表类型的类型，也代表它自己的类型

# 类型可用于文档化代码、执行优化以及多重派分(dispatch)
# Julia 并不只是静态的检查类型

# 用户可以自定义类型
# 就跟其他语言的 record 或 struct 一样
# 用 `struct` 关键字定义新的类型
...
# 像 Panther 一样使用内置构造函数，让你可以控制如何构造类型的值
# 应该尽量使用外部构造函数，而不是内部构造函数

####################################################
## 6. 多重派分(Multiple-Dispatch)
####################################################
